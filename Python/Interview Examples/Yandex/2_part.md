...
- пример использования в DNS
  ...
  - использует UDP, 53 порт
  - пример коммуникации ![img.png](images/img.png)
  - если происходит ошибка, то срабатывает таймер, который ждет ответа от сервера, и если ответа нет, то заново присылает сообщение![img.png](images/img2.png)
...
- Существует и нарушение порядка сообщений ![img.png](images/img5.png)
- Оно решается с помощью механизма нумерации данных. Если сегменты придут не в том порядке, то получатель сам выставит их в нужном порядке.![img.png](images/img6.png)
- Если подтверждение не пришло от получателя, то отправитель шлет сегмент еще раз, но если у получателя он уже есть, то этот сегмент игнорируется и отправляется подтверждение ![img.png](images/img7.png)
